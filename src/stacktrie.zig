...
pub const StackTrieUnmanaged = struct {
    ...
    test "stacktrie.StackTrieUnmanaged.init creates root node" {
        var trie = try StackTrieUnmanaged.init(std.testing.allocator);
        defer trie.deinit(std.testing.allocator);

        try std.testing.expectEqual(1, trie.nodes.items.len);
        try std.testing.expectEqual(0, trie.nodes.items[StackTrieUnmanaged.RootId].hitCount);
    ...
    test "stacktrie.StackTrieUnmanaged.add kernel-only event creates correct trie structure" {
        var trie = try StackTrieUnmanaged.init(std.testing.allocator);
        defer trie.deinit(std.testing.allocator);
        ...
        try std.testing.expectEqual(1, trie.nodes.items[StackTrieUnmanaged.RootId].hitCount);

        // Deepest kernel frame (0xBBBB, added first since reversed) is child of root
        try std.testing.expectEqual(StackTrieUnmanaged.RootId, trie.nodes.items[1].parent);
    ...
    test "stacktrie.StackTrieUnmanaged.add duplicate kernel event increments hit counts" {
        var trie = try StackTrieUnmanaged.init(std.testing.allocator);
        defer trie.deinit(std.testing.allocator);
        ...
        try std.testing.expectEqual(2, trie.nodes.items[StackTrieUnmanaged.RootId].hitCount);
        try std.testing.expectEqual(2, trie.nodes.items[4].hitCount);
    }

    test "stacktrie.StackTrieUnmanaged.add different PIDs create distinct nodes for same IP" {
        var trie = try StackTrieUnmanaged.init(std.testing.allocator);
        defer trie.deinit(std.testing.allocator);
    ...
    test "stacktrie.StackTrieUnmanaged.reset preserves root, clears everything else" {
        var trie = try StackTrieUnmanaged.init(std.testing.allocator);
        defer trie.deinit(std.testing.allocator);
        ...
        try trie.nodes.append(std.testing.allocator, .{
            .hitCount = 5,
            .parent = StackTrieUnmanaged.RootId,
            .payload = .{ .kernel = .{ .kmapip = 0xdead } },
        });

        trie.nodes.items[StackTrieUnmanaged.RootId].hitCount = 10;

        trie.reset(std.testing.allocator);
