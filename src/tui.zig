...
pub fn getColorFromName(symbolName: []const u8, kind: SymbolTrie.TrieKind) vaxis.Color {
    ...
    return switch (kind) {
        // Root: A solid distinct dark red
        .root => .{ .rgb = .{ 180, 40, 40 } },

        // Kernel: Existing "Warm" Palette (Orange/Yellow/Red)
        .kernel => .{
            .rgb = .{
        ...
        // User: New "Cool" Palette (Blue/Cyan/Green)
        // We swap the Red and Blue components of the kernel logic to shift spectrum
        .user => .{
            .rgb = .{
                @as(u8, @intFromFloat(55.0 * v2)), // Low Red
                @as(u8, @intFromFloat(230.0 * v1)), // High/Var Green
                @as(u8, @intFromFloat(50.0 * v3)) + 205, // High Blue
            },
        },
    };
    // return .{
...
pub const Interface = struct {
    ...
    const FlamegraphBorderWEnd = 3;
    const FlamegraphBorderHBeg = 2;
    const FlamegraphBorderHEnd = 8;
    ...
    // Draw a border
    fn drawBorder(self: Interface, window: vaxis.Window, x1: u16, y1: u16, x2: u16, y2: u16, color: vaxis.Color) void {
    ...
    pub fn drawSymbol(
        ...
        var style = vaxis.Style{
            .fg = self.textColor,
            .bg = getColorFromName(symbol, std.meta.activeTag(entry.payload)),
            .bold = false,
        };
