...
static __always_inline int process_event(void *ctx) {
  // grab timestamp
  ...
  // if its pid 0, thats the kernel generating kernel events like scheduling,
  // skip
  ...
  long ures = bpf_get_stack(ctx, uscratch, MAX_STACK_DEPTH * sizeof(__u64),
                            BPF_F_USER_STACK);
  __u64 ustack_size = 0;
  if (ures > 0) {
    ustack_size = ures;
